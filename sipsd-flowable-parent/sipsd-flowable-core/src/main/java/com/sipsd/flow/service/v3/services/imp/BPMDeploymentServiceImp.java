...
//    private FormRepositoryService formRepositoryService;
    ...
    /**
     * 创建一个可运行的模型,根据模型的定义ID 
     * 本方法只是返回一个 reModel 中的对象，对象保存或操作，在发布模型中完成。
    ...
    public Model createRuModel(String modelId) throws UnsupportedEncodingException{
        ...
        Model resModel=repositoryService.createModelQuery().modelKey(defineModel.getKey()).modelVersion(defineModel.getVersion()).singleResult();
        if(resModel!=null){
            return resModel;
        }
    ...
    public  Map<String,Object>  deployWorkFloyByModeKey(String modelKey) throws Exception {
       /* List<Model> tmpModelList=repositoryService.createModelQuery().modelKey(modelKey).latestVersion().orderByLastUpdateTime().desc().list();
        Model tmpModel=null;
        if( CollectionUtils.isEmpty(tmpModelList)){
           return null;
        }else{
    ...
    public Map<String,Object>  deployWorkFloyByModeID(String modelId) throws Exception {
        ...
        Model repModel= repositoryService.createModelQuery().modelKey(modelData.getKey()).latestVersion().orderByLastUpdateTime().desc().singleResult();
        if(repModel==null){
       ...
       //TODO 判断流程中是否有form 表单
        ...
        if(StringUtils.isEmpty(tmpDelployID)){
           //TODO 判断是否存在已经发布的流程 如果要求更改之后 ，要让更改生效，本处就不要判断
            ...
            deployment=repositoryService.createDeploymentQuery().deploymentKey(modelData.getKey()).orderByDeploymenTime().desc().latest().singleResult();
            if(deployment==null){
                DeploymentBuilder deploymentBuilder = repositoryService.createDeployment().tenantId(modelData.getTenantId()).name(modelData.getName()).key(modelData.getKey()).
...
//                .deploy();
     ...
     */
    @Override
    public ResultListDataRepresentation getModels(String tenantId, String filter, String sort, Integer modelType, HttpServletRequest request) {
        ...
        if (validFilter != null) {
            models = modelRepository.findByTenantModelTypeAndFilter(modelType, tenantId,validFilter, sort);

        } else {
            models = modelRepository.findByTenantModelType(modelType, tenantId,sort);
        }
