 ...
 * @author 高强
 * @title: ChinaDate
 * @projectName sipsd-holiday
 * @description: TODO
 * @date 2021/10/13下午3:59
 */
public class ChinaDate
{
    ...
    public static String solarToLunar(String solarDate, Boolean isFormat) throws Exception
    {
        ...
        int temp = 0;
        int lunarYear;
        String lunarMonth; //农历月份
        String lunarDay; //农历当月第几天
        ...
        offset += temp;
        lunarMonth = String.valueOf(i).length()==1?"0"+String.valueOf(i):String.valueOf(i);
        lunarDay = String.valueOf(offset).length()==1?"0"+String.valueOf(offset):String.valueOf(offset);

        if (isFormat)
            return "阴历：" + lunarYear + "年" + (leapMonthFlag & (lunarMonth.equals(leapMonth)) ? "闰" : "") + lunarMonth + "月" + lunarDay + "日";
        return "" + lunarYear + lunarMonth + lunarDay;
    }

    public static void main(String[] args) throws Exception
    {
        String dateCode = solarToLunar("2021-02-26", false);
        System.out.println(dateCode);
        SimpleDateFormat sdf = new SimpleDateFormat("yyyyMMdd");
        Date date = sdf.parse(dateCode);
