...
angular.module('flowableModeler')
  .controller('ProcessCtrl', ['$rootScope', '$scope', '$translate', '$http', '$location', '$routeParams','$modal', '$popover', '$timeout', 'appResourceRoot', 'ResourceService',
    ...
    $scope.publishModel = function() {
        //$http({method: 'GET', url: FLOWABLE.APP_URL.getPublishModel($scope.model.process.key)}).
    	$http({method: 'POST', url: FLOWABLE.APP_URL.getPublishModel($scope.model.process.id)}).
        success(function(data, status, headers, config) {
            $scope.returnToList();
        });
    }
