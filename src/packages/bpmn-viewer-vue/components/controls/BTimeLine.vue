...
export default {
  ...
  data(){
    ...
    return {
      ...
      oldStyle:{color:null,setline,shadow:false,stroke:true},
      highLight:[
        {color:null,setline,shadow:true,stroke:true},
        {color:null,setline,shadow:true,stroke:true},
        {color:null,setline,shadow:true,stroke:true},
        {color:null,setline,shadow:true,stroke:true}
      ],
  ...
  methods:{
    ...
    handleItemOver(item){
      ...
      if(taskObj){
        let hexColor = utils.rgbToHex(taskObj.color)
        item.oldStyle = Object.assign({},this.oldStyle, {color:hexColor})
      }
      this.bpmnOptions.additionalModules[0].utils.setTaskHighlight(this.bpmnViewer._container,[item.taskDefinitionKey],
          Object.assign({},this.highLight[type-1]),
      )
    },
    handleItemOut(item){
