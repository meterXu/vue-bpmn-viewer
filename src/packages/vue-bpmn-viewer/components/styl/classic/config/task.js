...
function userObj(options){
    ...
    if(options.businessObject&&options.businessObject.extensionElements){
        ...
        let assignee = '-'
        if(options.businessObject.extensionElements&&options.businessObject.extensionElements.values&&options.businessObject.extensionElements.values.find(c=>c.$type==='customs:assignee-info-lastname')){
            assignee =options.businessObject.extensionElements.values.find(c=>c.$type==='customs:assignee-info-lastname').$body
        }
        if(userInfoLastName){
...
function createCon(options,text,clock){
    ...
    <ul style="max-width: ${options.width-6}px">
        <li class="d-con-user">
            ...
            <span title="${text}" style="color:${text?'#6c6c6c':''}">${text}</span>
        </li>
        ${clockLi}
    </ul>`
...
function createUnConText(option={x:20,y:50,text:'-'}){
    let foreignObject = create('foreignObject',{
        width:svgWH.width,
        height:svgWH.height
    })
    let div = document.createElement("div");
    div.className = "custom-realName-con"
